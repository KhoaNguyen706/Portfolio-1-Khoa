...
export default function HeroSection() {
  return (
    <section className="relative flex min-h-screen flex-col items-center justify-center px-4 pt-32 pb-20">
      <div className="z-10 w-full max-w-5xl text-center space-y-8">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-xs font-semibold tracking-wider text-accent uppercase animate-fade-in">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-accent"></span>
          </span>
          Software Engineer Intern @ SCE · Open for internships
        </div>
        ...
        <h1 className="text-5xl md:text-8xl font-black tracking-tight text-foreground leading-[1.1]">
          ...
          <span className="relative inline-block">
            ...
            {/* Orbiting Space Shuttle (Dark Mode Only) */}
            <div className="absolute inset-0 hidden dark:block pointer-events-none animate-[spin_8s_linear_infinite] z-20">
              {/* The shuttle SVG is offset to create the orbit radius */}
        ...
        <p className="max-w-2xl mx-auto text-xl md:text-2xl font-medium text-foreground/60 leading-relaxed">
        ...
        <div className="pt-12 flex flex-col items-center gap-8">
          <div className="bento-card inline-block">
            <p className="text-sm font-semibold text-foreground/40 uppercase tracking-widest mb-2">Current Age</p>
            <AgeCounter />
          </div>
          ...
          <div className="flex items-center gap-4">
            <a 
              href="#projects" 
              className="px-8 py-4 rounded-full bg-foreground text-background font-bold transition-all hover:scale-105 active:scale-95 shadow-xl"
            >
              View My Work
            </a>
            <a 
              href="#about" 
              className="px-8 py-4 rounded-full glass font-bold transition-all hover:bg-foreground/5 active:scale-95"
            >
              About Me
            </a>
          </div>
